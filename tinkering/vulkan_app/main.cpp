...
// TODO: Start building engine making vulkan_app project the entry point??? (add more things to it with time)
// TODO: Change vk objects functions arguments from pointer to value if possible (mainly in destructors I think)

#define GET_ASSET_PATH(asset_name) "../../../tinkering/vulkan_app/" asset_name

int main()
{
    ti::graphics::initalize();

    while(!ti::graphics::shouldClose())
    {
        ti::graphics::pollEvents();
        ti::graphics::drawFrame();
    }

    ti::graphics::terminate();
    return 0;
}
